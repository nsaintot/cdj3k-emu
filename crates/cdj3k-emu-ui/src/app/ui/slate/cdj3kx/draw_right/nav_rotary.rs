...
use super::super::{
    ...
    COL_BLACK, COL_BTN, COL_BTN_TEXT, COL_DARK, COL_SILVER, NAV_DETENT_COUNT,
    NAV_SCROLL_PX_PER_TICK,
};
use super::*;
use crate::app::ui::draw_vinyl_speed::{
...
pub(in crate::app) const NAV_BEZEL_INNER_GAP_REF: f32 = 12.0;
/// Width (ref units) of the filled bezel ring between gear and buttons.
pub(in crate::app) const NAV_BEZEL_WIDTH_REF: f32 = 70.0;
...
/// Maximum total widening of the ring stroke for the outermost glow layer (ref
/// units). The lit ring sits 127 out and the block's side wall at 160, so the
/// outermost layer reaches 156 - inside the pod, and clipped to it regardless.
...
pub(super) fn draw_nav_rotary(
    ...
    if resp.hovered() {
        ...
        if scroll_y != 0.0 {
            ...
            while app.nav_scroll_accum >= NAV_SCROLL_PX_PER_TICK {
                app.nav_scroll_accum -= NAV_SCROLL_PX_PER_TICK;
                app.nav_angle += detent_rad;
                app.rotary = app.rotary.wrapping_sub(1);
                app.inject_rotary();
            }
            while app.nav_scroll_accum <= -NAV_SCROLL_PX_PER_TICK {
                app.nav_scroll_accum += NAV_SCROLL_PX_PER_TICK;
                app.nav_angle -= detent_rad;
                app.rotary = app.rotary.wrapping_add(1);
                app.inject_rotary();
            }
        }
    }
    ...
    if let Some(bit) = app.button(Btn::RotaryPress) {
    ...
    // The ring is cut flat at the button block's vertical sides: the arc quads
    // carry their inner arc's x out to the outer radius, so those sides stand
    // at bezel_outer_r * sin(a), inside the ring's own widest point.
    // The block's side is a vertical path at bezel_outer_r * sin(a), and its
    // border stroke is centred on that path, so the block's silhouette stands
    // half a stroke further out. Cut the ring there, not at the path.
    ...
    let bezel_clip = {
        ...
        p.with_clip_rect(full.intersect(egui::Rect::from_min_max(
            egui::Pos2::new(center.x - bezel_half_w, full.top()),
            egui::Pos2::new(center.x + bezel_half_w, full.bottom()),
        )))
    };
    let bezel_thickness = bezel_outer_r - bezel_inner_r;
    bezel_clip.circle_stroke(
        center,
        bezel_inner_r,
        Stroke::new(bezel_thickness, COL_BLACK),
    );
    let bezel_outline = Stroke::new(layout.sc(NAV_BEZEL_STROKE_REF), COL_SILVER);
    bezel_clip.circle_stroke(center, bezel_outer_r, bezel_outline);
    bezel_clip.circle_stroke(center, bezel_inner_r, bezel_outline);
    // Each cut face carries the outline too, its outer edge flush with the
    // block's silhouette. A face is a chord of the outer circle, so its ends
    // land on the arc the clip stops short of.
    let face_x = bezel_half_w - bezel_outline.width * 0.5;
    let face_half_h = (bezel_outer_r * bezel_outer_r - face_x * face_x)
        .max(0.0)
        .sqrt();
    for side in [-1.0f32, 1.0] {
        let x = center.x + side * face_x;
        p.line_segment(
            [
                egui::Pos2::new(x, center.y - face_half_h),
                egui::Pos2::new(x, center.y + face_half_h),
            ],
            bezel_outline,
        );
    ...
    {
        // The selector ring is an RGB lamp of its own; the encoder bit only
        ...
        if encoder_on {
            ...
            // The lamp rides the bezel ring, which the button block cuts flat
            // at its sides - so the light is cut there too.
            ...
            bezel_clip.circle_stroke(center, bezel_inner_r, Stroke::new(main_w, glow_col));
        }
    }
    ...
    let nav_pressed = resp.is_pointer_button_down_on()
        || app
            .button(Btn::RotaryPress)
            .is_some_and(|bit| app.latched_btns.contains(&bit));
