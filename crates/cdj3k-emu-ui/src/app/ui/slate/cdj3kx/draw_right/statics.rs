...
use crate::app::ui::draw_vinyl_speed::{
    VINYL_SPEED_ENDSHAPE_ARM_W, VINYL_SPEED_ENDSHAPE_GAP_Y, VINYL_SPEED_ENDSHAPE_H,
    VINYL_SPEED_ENDSHAPE_INSET_X, VINYL_SPEED_ENDSHAPE_STROKE, VINYL_SPEED_KNOB_RADIUS_REF,
    VINYL_SPEED_LABEL_GAP_REF, VINYL_SPEED_RAMP_ARM_FRAC, VINYL_SPEED_SIZE_SCALE,
    VINYL_SPEED_TICK_ARC_SPAN_RAD, VINYL_SPEED_TICK_ARC_START_CW_RAD,
    VINYL_SPEED_TICK_RING_OFFSET_REF, VINYL_SPEED_TOOTH_AMP_REF, VINYL_SPEED_TOP_LABEL_FONT_SIZE,
};
...
pub(super) fn collect_right_statics(list: &mut ShapeList, ctx: &egui::Context, layout: &UiScale) {
    ...
    {
        let combo_ref = jog_mode_boxes().0;
        let combo_screen = layout.sr(
            combo_ref.left(),
            combo_ref.top(),
            combo_ref.width(),
            combo_ref.height(),
    ...
    // ── "INST. DOUBLES" legend, behind the drawn dash ─────────────────────────
    {
        let left_ref = col.left() + col.width() * SYNC_INST_DOUBLES_U;
        let cy_ref = col.top() + col.height() * SYNC_INST_DOUBLES_V;
        list.rect_filled(
            Rect::from_center_size(
                layout.sp(left_ref - SYNC_INST_DOUBLES_TAB_GAP, cy_ref),
    ...
    {
        let section_ref = sync_boxes().0;
        let section_screen = layout.sr(
            section_ref.left(),
            section_ref.top(),
            section_ref.width(),
            section_ref.height(),
    ...
    list.text(
        ctx,
        layout.sp_in_rect(col, 0.513, TEMPO_RANGE_LABEL_V),
    ...
    list.text(
        ctx,
        layout.sp_in_rect(col, 0.513, TEMPO_RANGE_SUBLABEL_V),
        Align2::CENTER_CENTER,
        "±6/±10/±16/WIDE",
    ...
    list.text(
        ctx,
        layout.sp_in_rect(col, 0.510, MASTER_TEMPO_LABEL_V),
        Align2::CENTER_CENTER,
        "MASTER\n TEMPO",
    ...
    // ── Tempo slider: backdrop, label, track bars, tick marks & segments ───────
    {
        ...
        for i in 0..TEMPO_TICK_COUNT {
            ...
            // The CDJ-3000X has no ruled scale across the fader; only the side column
            // of detents with its - / 0 / + marks.
    ...
    // ── VINYL SPEED ADJ. labels + endstop brackets ────────────────────────────
    {
        ...
        let center = VINYL_SPEED_PLACE.center(layout);
    ...
    // ── Bottom label ──────────────────────────────────────────────────────────
    // The CDJ-3000X carries the model name alone - no "MULTI PLAYER" line.
    {
        list.text(
            ctx,
            layout.sp(
