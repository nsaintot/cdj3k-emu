 ...
 * fabricate DRM objects matching what the app's jog assembly expects.
 *
...
#define SHIM_GETCONNECTOR_IOC 0xC05064A7u
...
static drm_api_connector_t *_get_connector_real(int fd, uint32_t id) {
    ...
    if (sys_ioctl(fd, SHIM_GETCONNECTOR_IOC, &kc) < 0)
        return NULL;
...
#define DRM_MODE_CONNECTOR_DSI_TYPE 16u
    if (c->connector_type == DRM_MODE_CONNECTOR_DSI_TYPE &&
        ...
        if (syn) {
            ...
            c->count_modes = 1;
            fprintf(stderr,
                    "[deck_shim] drmModeGetConnector(id=%u): synthesized "
...
drm_api_encoder_t *drmModeGetEncoder(int fd, uint32_t encoder_id) {
    ...
    if (sys_ioctl(fd, SHIM_GETENCODER_IOC, &ke) < 0) {
    ...
    if (e->crtc_id == 0 && (e->possible_crtcs & 2u)) {
        ...
        if (sys_ioctl(fd, SHIM_GETRESOURCES_IOC, &res) == 0 &&
            res.count_crtcs >= 2) {
            ...
            if (crtc_ids) {
                ...
                if (sys_ioctl(fd, SHIM_GETRESOURCES_IOC, &res) == 0) {
                    ...
                    fprintf(stderr,
                            "[deck_shim] drmModeGetEncoder: GETRESOURCES phase2 failed: %s\n",
                            strerror(errno));
                }
            ...
            fprintf(stderr,
                    "[deck_shim] drmModeGetEncoder: GETRESOURCES phase1 failed or <2 CRTCs: %s\n",
                    strerror(errno));
        }
    }

    fprintf(stderr,
            "[deck_shim] drmModeGetEncoder(id=%u) type=%u crtc_id=%u possible=0x%x\n",
