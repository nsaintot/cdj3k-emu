...
static int do_open(const char *pathname, int flags, mode_t mode) {
    if (pathname) {
         ...
         * The app's MainDisplayUpdateTimer polls it at ~30 Hz to pace rendering.
         ...
         * The GPIO hardware does not exist in QEMU virt; each open gets an fd
         * backed by the stub pipe, and read() reports GPIODRV_STATE_BYTE. */
        ...
        /* The model's touch device, on a model that reads one: an input node
         * QEMU virt does not have, answered from the sub-CPU frame.  The
         * model directory owns it; nothing here knows the device. */
        if (deck_touch_owns_path && deck_touch_owns_path(pathname)) {
            int fd = deck_touch_open(flags);
            DBG("open(%s, 0x%x) → touch stub fd %d\n", pathname, flags, fd);
            return fd;
        }
        ...
        if (strcmp(pathname, SUBUCOM_SPI_PATH) == 0) {
            ...
            if (fd >= 0) {
                ...
                DBG("open(%s, 0x%x) → subucom fd %d (passthrough)\n",
                    pathname, flags, fd);
            }
            return fd;
        }
         ...
         * The app writes the USB interface name to these sysfs entries to bind or
         * unbind the USB storage driver.  In QEMU virt there is no xHCI
         * controller so the real sysfs write would fail ENODEV causing the app
         ...
         * The app's OverheatingWatcher reads this periodically; without it the app
         * logs an error every tick.  Return a one-shot pipe pre-seeded with
         * "40000\n" - the app opens a new fd each read, so re-seeding is correct. */
         ...
         * The app sets screen brightness; no backlight hw exists in QEMU. */
...
/* close                                                              */
/* ------------------------------------------------------------------ */
/* Defined in link.c. Returns 1 if the FD has a Pro DJ Link packet pending
 * in the delayed-send queue (in which case we must NOT close yet - the link
 * worker will close it after firing the packet). */
extern int shim_link_intercept_close(int fd);
...
int close(int fd) {
    ...
    if (deck_touch_is_fd && deck_touch_is_fd(fd)) {
        deck_touch_close(fd);
        DBG("close(touch fd %d)\n", fd);
        return sys_close(fd);
    }
    if (fd == __atomic_load_n(&g_subucom_fd, __ATOMIC_ACQUIRE))
        __atomic_store_n(&g_subucom_fd, -1, __ATOMIC_RELEASE);
    /* Pro DJ Link delayed-send: defer the kernel close until our worker
     * has fired the queued packet on this FD. */
    if (shim_link_intercept_close(fd)) {
        return 0;   /* tell the app close succeeded; worker will close later */
    }
    return sys_close(fd);
}
...
void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset) {
    ...
    /* Log every MAP_SHARED mmap on a DRM fd so we can see ALL of the app's buffer maps. */
    ...
    /* Snoop the app's DRM mmap: when it maps a jog GEM buffer (matched by mmap_offset
...
int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
     ...
     *
     * subucom/pcm: must be forced POLLIN|POLLOUT so the app's tight
     ...
     *   poll), we fall through to raw_poll with a 33 ms cap so the app
     ...
     *
     * IMPORTANT: the app's JUCE Message Thread polls the DRM fd AND the X11
    ...
    /* the app's gadget manager treats a hidg fd that never reports POLLOUT as a
