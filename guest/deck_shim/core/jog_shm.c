...
/* Register a jog framebuffer handle so we can later snoop the app's MAP_DUMB
...
static void jog_register_fb(uint32_t fb_id, uint32_t gem_handle)
{
    ...
    g_jog_fb_count++;
    fprintf(stderr, "[deck_shim] jog fb_id=%u handle=%u registered (waiting for the app mmap)\n",
            fb_id, gem_handle);
}
 ...
 *
 * The app writes a 4:1 horizontally-stretched buffer where logical pixel C occupies
...
static void *open_ivshmem_bar(void)
{
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        if (fd < 0) {
            fprintf(stderr, "[deck_shim] open %s: %s\n", path, strerror(errno));
            continue;
        }
        map = mmap(NULL, JOG_SHM_BAR_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        close(fd);
        if (map == MAP_FAILED) {
            fprintf(stderr, "[deck_shim] mmap ivshmem BAR2: %s\n", strerror(errno));
            map = NULL;
            continue;
        }
        fprintf(stderr, "[deck_shim] ivshmem BAR2 mapped at %p (PCI %s)\n", map, de->d_name);
        break;
    }
    closedir(d);
    return map;
...
static void jog_shm_init_once(void)
{
    ...
    if (!base) {
        fprintf(stderr, "[deck_shim] ivshmem device not found - jog stream disabled\n");
        return;
    }
    ...
    fprintf(stderr, "[deck_shim] jog ivshmem ready: pixels @ %p (%ux%u XRGB8888)\n",
 ...
 * the handle is a valid dma-buf), MAP_DUMB+mmap it inside the app so we can read
...
void export_jog_prime(int drm_fd, uint32_t gem_handle, uint32_t fb_id)
{
    ...
    if (sys_ioctl(drm_fd, 0xC00C642Du, &prime) < 0)
        fprintf(stderr, "[deck_shim] PRIME_HANDLE_TO_FD(handle=%u): %s (non-fatal)\n",
                gem_handle, strerror(errno));
    else {
        if (g_jog_prime_fd >= 0) sys_close(g_jog_prime_fd);
        g_jog_prime_fd = prime.fd;
        fprintf(stderr, "[deck_shim] jog PRIME fd=%d (handle=%u fb_id=%u)\n",
                prime.fd, gem_handle, fb_id);
    }

    /* Register this handle, then immediately MAP_DUMB + mmap it ourselves.
     * We are running inside the app's process (the DRM master), so this is safe -
    ...
    jog_register_fb(fb_id, gem_handle);
    {
        ...
        if (sys_ioctl(drm_fd, 0xC01064B3u /* DRM_IOCTL_MODE_MAP_DUMB = _IOWR('d',0xB3,16) */, &md) == 0) {
            ...
            fprintf(stderr, "[deck_shim] MAP_DUMB(master) handle=%u → offset=0x%llx\n",
            ...
            if (ptr != MAP_FAILED) {
                ...
                fprintf(stderr, "[deck_shim] mmap(master) jog handle=%u → %p\n", gem_handle, ptr);
            } else {
                fprintf(stderr, "[deck_shim] mmap(master) jog handle=%u: %s\n", gem_handle, strerror(errno));
            }
        } else {
            fprintf(stderr, "[deck_shim] MAP_DUMB(master) handle=%u: %s\n", gem_handle, strerror(errno));
        }
    }
