...
| `soc_serial` | 16 hex digits | generated | The guest's `/proc/cpuinfo` `Serial` line. Minted on first load, regenerated by a firmware install, fixed otherwise - see below. |
...
7. `InstanceSettings::regenerate_soc_serial` mints the slot's SoC serial,
   or `set_soc_serial` pins it from `CDJ3K_SOC_SERIAL`. The staged cabinet
